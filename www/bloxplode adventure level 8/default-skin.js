...
VisualPipe.on("start_sequence", ({ gold, purple, red }) => {
    // 1. Get Elements
    ...
    const redCountUI = document.getElementById('red-count'); // NEW: Added Red UI
    ...
    if (redCountUI) redCountUI.classList.add('socket-empty'); // NEW: Hide Red
    ...
    // NEW: Red Banner Block
    if (red > 0) {
        htmlContent += `
            <div class="start-gem-wrapper" id="start-gem-red-wrapper">
                <div class="gem-icon gem-red"></div>
                <span>${red}</span>
            </div>
        `;
    }
    ...
    setTimeout(() => {
        ...
        if (red > 0) types.push('red'); // NEW: Push Red

        // --- PHASE 3: GHOST CREATION ---
        types.forEach(type => {
            const sourceWrapper = document.getElementById(`start-gem-${type}-wrapper`);
            
            // NEW: Target Logic including Red
            let targetId = 'gold-count';
            if (type === 'purple') targetId = 'purple-count';
            if (type === 'red') targetId = 'red-count';
        ...
        setTimeout(() => {
            ...
            setTimeout(() => {
                ...
                // 2. Identify Targets (Gold/Purple/Red Containers)
                const targets = [];
                if (goldCountUI) {
                ...
                // NEW: Add Red Target to Animation Array
                if (redCountUI) {
                    redCountUI.classList.remove('socket-empty');
                    targets.push(redCountUI.parentElement);
                }
